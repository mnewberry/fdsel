...
let rec fold5 kons knil la lb lc ld le = match (la, lb, lc, ld, le) with 
    ([], [], [], [], []) -> knil
  | ((ha :: ta), (hb :: tb), (hc :: tc), (hd :: td), (he :: te)) -> 
    fold5 kons (kons ha hb hc hd he knil) ta tb tc td te ;
  | _ -> invalid_arg "unequal lengths" ;;
...
let iter5 f la lb lc ld le = let kons a b c d e () = f a b c d e in 
  fold5 kons () la lb lc ld le ;;
let app m n = fold cons n (rev m) ;;
let app_sloppy m n = fold cons n m ;;
...
let filtmap f l = 
  ...
  fold kons [] (rev l) ;;
let filtmap2 f la lb = 
  let kons kara karb kdr = match f kara karb with Some x -> x :: kdr 
    | None -> kdr in
  fold2 kons [] (rev la) (rev lb) ;;
let sfiltmap f l = 
  let kons kar kdr = match f kar with Some x -> x :: kdr | None -> kdr in
...
let mean ll = let (s, n) = fold (fun x (s, n) -> (s+x,n+1)) (0,0) ll in 
  float_of_int s /. float_of_int n
let meanf ll = let (s, n) = fold (fun x (s, n) -> (s+.x,n+1)) (0.,0) ll in
  s /. float_of_int n
let meanf_n ll = let (s, n) = fold (fun x (s, n) -> 
    if compare x nan == 0 then (s,n) else (s+.x,n+1)) (0.,0) ll in 
  s /. float_of_int n
